...
bj$year <- as.factor(year(bj$date))
bj$month <- month(bj$date)
bj$month <- as.factor(bj$month)
bj$ym <- as.factor(bj$year:bj$month)
bj$tot <- bj$A00toR99
bj$cir <- bj$I00toI99
bj$resp <- bj$J00toJ99
...
par(mfrow = c(3, 1))
br <- seq(0, 400, 10)
hist(bj$ave_pm, breaks = br, main = "Year-round", 
     xlab = expression(paste(PM[2.5], " concentration", 
                        " (", mu,"g/", m^3, ")", sep = " ")), 
     xlim = c(10, 400), ylim = c(0, 230),
     cex.axis = 1.2, cex.lab = 1.2, cex.main = 1.5)
abline(v = 35, lty = 3, lwd = 2.5)
abline(v = 75, lwd = 2.5)
hist(bj.cold$ave_pm, breaks = br, main = "Cold Season",
     xlab = expression(paste(PM[2.5], " concentration", 
                        " (", mu,"g/", m^3, ")", sep = " ")), 
     xlim = c(10, 400), ylim = c(0, 230),
     cex.axis = 1.2, cex.lab = 1.2, cex.main = 1.5)
abline(v = 35, lty = 3, lwd = 2.5)
abline(v = 75, lwd = 2.5)
hist(bj.warm$ave_pm, breaks = br, main = "Warm Season",
     xlab = expression(paste(PM[2.5], " concentration", 
                        " (", mu,"g/", m^3, ")", sep = " ")), 
     xlim = c(10, 400), ylim = c(0, 230),
     cex.axis = 1.2, cex.lab = 1.2, cex.main = 1.5)
...
# linear model
a.l.cb <- cb.lin(bj)
w.l.cb <- cb.lin(bj.warm, group = bj.warm$year)
c.l.cb <- cb.lin(bj.cold, group = bj.cold$gr)
# spline model
a.s.cb <- cb.spl(bj, a.knots)
w.s.cb <- cb.spl(bj.warm, w.knots, group = bj.warm$year)
...
The effects of PM2.5 on circulatory death are still high at the higher exposure levels of PM2.5.
...
The effects of PM2.5 on circulatory death are leveling off at the higher exposure levels of PM2.5.
...
year_round <- c(rr.fun(data = bj, cause = "tot", df = 6), rr.fun(data = bj, cause = "cir", df = 6), 
                rr.fun(data = bj, cause = "resp", df = 6))
...
attr.lin <- function(data = c(), cold = c(), warm = c(), cause = "tot", cen = c()){
  cb <- cb.lin(data = data)
  model <- mod(data = data, cause = cause, cb = cb, df = 6) # model fits year-round data
...
```{r warning=FALSE}
require(splines)
l1 <- attr.lin(data = bj, cold = bj.cold, warm = bj.warm, cen = 7.5)
l2 <- attr.lin(data = bj, cold = bj.cold, warm = bj.warm, cen = 25)
l3 <- attr.lin(data = bj, cold = bj.cold, warm = bj.warm, cen = 75)
ll <- cbind(l1, l2, l3)
rownames(ll) <- c("lin-year", "lin-cold", "lin-warm")

s1 <- attr.spl(data = bj, cold = bj.cold, warm = bj.warm, cen = 7.5)
s2 <- attr.spl(data = bj, cold = bj.cold, warm = bj.warm, cen = 25)
s3 <- attr.spl(data = bj, cold = bj.cold, warm = bj.warm, cen = 75)
ss <- cbind(s1, s2, s3)
